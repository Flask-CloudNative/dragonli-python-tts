...
@app.route('/dragonli/1.0/tts', methods=['GET', 'POST', 'PUT', 'DELETE'])
def api():
    parameter = Parameter(request)

    if parameter.method == 'GET':
        if parameter.verification(checking=parameter.param_url, verify={'text': str}):
            param = parameter.param_url
            # 根据文档是否存在
            if db.does_it_exist(docu={'text': param['text']}):
                documen = db.mongo_collection.find_one({'text': param['text']})
                # 返回.wav音频
                return Response(documen['wav'], mimetype="audio/x-wav")
            else:
                tts = ToSpeech()
                # TTS操作
                stream = tts.recognize(text=param['text'])
                # 文件保存操作
